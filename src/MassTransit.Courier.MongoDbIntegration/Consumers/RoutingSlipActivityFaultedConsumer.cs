...
namespace MassTransit.Courier.MongoDbIntegration.Consumers
{
    ...
    public class RoutingSlipActivityFaultedConsumer :
        Consumes<RoutingSlipActivityFaulted>.Context
    {
        readonly EventPersister _persister;

        public RoutingSlipActivityFaultedConsumer(EventPersister persister)
        {
            _persister = persister;
        }

        public void Consume(IConsumeContext<RoutingSlipActivityFaulted> context)
        {
